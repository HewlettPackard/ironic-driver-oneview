...
def set_boot_device(driver_info, new_primary_boot_device):
    ...
    return prepare_and_do_request(uri=server_profile_dict.get('uri'),
...
def clone_and_assign(driver_info, server_profile_template_uri, instance_uuid,
                     max_retries=CONF.oneview.max_retries):
    LOG.debug('Cloning server profile %s on OneView',
              server_profile_template_uri)

    server_profile_template = get_server_profile_template(driver_info)
    server_hardware = get_server_hardware(driver_info)
    server_profile_name = "Ironic [%s]" % (instance_uuid)
    server_profile_clone_body = _build_clone_body(driver_info,
                                                  server_profile_template,
                                                  server_hardware,
                                                  server_profile_name)
    ret = prepare_and_do_request(uri="/rest/server-profiles",
